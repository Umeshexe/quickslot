...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // lock this row so two concurrent requests can't both read 'available'
    ...
    const bookingResult = await client.query(
      `INSERT INTO bookings (user_id, slot_id) VALUES ($1, $2) RETURNING id, user_id, slot_id, booked_at`,
      [userId, slotId]
    );
